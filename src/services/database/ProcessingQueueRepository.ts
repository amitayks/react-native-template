...
export class ProcessingQueueRepository {
	 ...
	 *
	 * Uses database.batch() for true atomic batch operations
	 */
	static async createBatch(
		dataArray: CreateProcessingQueueData[],
	): Promise<ProcessingQueue[]> {
		const preparedRecords = await database.write(async () => {
			// Prepare all records for batch insertion
			const recordsToCreate = dataArray.map((data) =>
				database.get<ProcessingQueue>("processing_queue").prepareCreate((record) => {
			...
			// Batch insert ALL records in ONE atomic operation
